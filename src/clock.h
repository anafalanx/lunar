...
//
// Before any successful sync THIS run, Clock_NowUtcMs() refuses to
// return a time (returns 0). The Windows system clock is never used
// as a display source; callers must render an "INOP" indicator in
// that case.
...
// clockwork has NOT been anchored by a successful NTP sync this run,
// returns 0 and leaves *outMs untouched. Callers MUST check the return
// value -- the Windows system clock is never used as a display fallback.
...
// with the concurrence verdict computed from the three sources:
//   TRUST_OK   -- all 3 sources reached and all pairwise spreads <= 200 ms
//   TRUST_INOP -- anything else: 0/1/2 reached, or 3 reached but any pair
//                 disagrees by more than 200 ms, or the agreed time
//                 disagrees with our running projection by more than
//                 200 ms (suggests local oscillator fault).
//
...
// unanimous concurrence among all three national-metrology sources
// counts as trusted; there is no degraded middle ground.
typedef enum {
    TRUST_INOP = 0,
    TRUST_OK   = 1,
} TrustState;
